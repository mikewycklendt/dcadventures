...
def power_medium_subtype_select():
	...
	medium_subtypes = db.session.query(MediumSubType).filter(MediumSubtype.medium_type=medium_type).order_by(MediumSubType.name).all()
...
def power_medium_select():
	...
	try:
		...
		mediums = db.session.query(Medium).filter_by(medium_type=medium_type, medium_subtype=medium_subtype).order_by(name).all()
