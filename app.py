...
def table_db_columns_create():
	...
	entry = Equipment(all=True, name=name, show=False )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Current ' + tablename

	entry = Equipment(current=True, name=name, show=False )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Any ' + tablename

	entry = Equipment(any=True, name=name, show=False )
	db.session.add(entry)
	db.session.commit()

	name = 'Variable ' + tablename

	entry = Equipment(var=True, name=name, show=False )
	db.session.add(entry)
	db.session.commit()
	
	name = 'No ' + tablename

	entry = Equipment(none=True, name=name, show=False )
	db.session.add(entry)
	db.session.commit()

	results = db.session.query(Equipment).filter_by(show=False).all()
...
def time_db_columns_create():
	...
	entry = PowerTime(perm=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()

	name = 'Round'
	
	entry = PowerTime(round=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()

	name = 'Scene'

	entry = PowerTime(scene=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()

	name = 'Instant'

	entry = PowerTime(instant=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()

	name = '1 Turn'

	entry = PowerTime(turn=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Next Round'

	entry = PowerTime(next=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Set by GM'

	entry = PowerTime(gm=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Set by Player'

	entry = PowerTime(player=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()
	
	name = 'Maintaining Action'

	entry = PowerTime(maintain=True, keyword=name, hide=True )
	db.session.add(entry)
	db.session.commit()
